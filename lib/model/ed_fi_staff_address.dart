...
class EdFiStaffAddress {
  /// Returns a new [EdFiStaffAddress] instance.
  EdFiStaffAddress({
  ...
  List<EdFiStaffAddressPeriod> periods;

  @override
  bool operator ==(Object other) => identical(this, other) || other is EdFiStaffAddress &&
  ...
  /// Returns a new [EdFiStaffAddress] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static EdFiStaffAddress? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "EdFiStaffAddress[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "EdFiStaffAddress[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return EdFiStaffAddress(
        ...
        periods: EdFiStaffAddressPeriod.listFromJson(json[r'periods']),
      );
    }
    return null;
  }

  static List<EdFiStaffAddress> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <EdFiStaffAddress>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = EdFiStaffAddress.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, EdFiStaffAddress> mapFromJson(dynamic json) {
    final map = <String, EdFiStaffAddress>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = EdFiStaffAddress.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of EdFiStaffAddress-objects as value to a dart map
  static Map<String, List<EdFiStaffAddress>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<EdFiStaffAddress>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = EdFiStaffAddress.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }

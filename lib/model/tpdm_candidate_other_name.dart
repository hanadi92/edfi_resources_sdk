...
class TpdmCandidateOtherName {
  /// Returns a new [TpdmCandidateOtherName] instance.
  ...
  @override
  bool operator ==(Object other) => identical(this, other) || other is TpdmCandidateOtherName &&
  ...
  @override
  String toString() => 'TpdmCandidateOtherName[otherNameTypeDescriptor=$otherNameTypeDescriptor, firstName=$firstName, generationCodeSuffix=$generationCodeSuffix, lastSurname=$lastSurname, middleName=$middleName, personalTitlePrefix=$personalTitlePrefix]';
  ...
  /// Returns a new [TpdmCandidateOtherName] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static TpdmCandidateOtherName? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "TpdmCandidateOtherName[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "TpdmCandidateOtherName[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return TpdmCandidateOtherName(
  ...
  static List<TpdmCandidateOtherName> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <TpdmCandidateOtherName>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = TpdmCandidateOtherName.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, TpdmCandidateOtherName> mapFromJson(dynamic json) {
    final map = <String, TpdmCandidateOtherName>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = TpdmCandidateOtherName.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of TpdmCandidateOtherName-objects as value to a dart map
  static Map<String, List<TpdmCandidateOtherName>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<TpdmCandidateOtherName>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = TpdmCandidateOtherName.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }

...
class EdFiContactAddress {
  /// Returns a new [EdFiContactAddress] instance.
  EdFiContactAddress({
  ...
  List<EdFiContactAddressPeriod> periods;

  @override
  bool operator ==(Object other) => identical(this, other) || other is EdFiContactAddress &&
  ...
  /// Returns a new [EdFiContactAddress] instance and imports its values from
  /// [value] if it's a [Map], null otherwise.
  // ignore: prefer_constructors_over_static_methods
  static EdFiContactAddress? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        requiredKeys.forEach((key) {
          assert(json.containsKey(key), 'Required key "EdFiContactAddress[$key]" is missing from JSON.');
          assert(json[key] != null, 'Required key "EdFiContactAddress[$key]" has a null value in JSON.');
        });
        return true;
      }());

      return EdFiContactAddress(
        ...
        periods: EdFiContactAddressPeriod.listFromJson(json[r'periods']),
      );
    }
    return null;
  }

  static List<EdFiContactAddress> listFromJson(dynamic json, {bool growable = false,}) {
    final result = <EdFiContactAddress>[];
    if (json is List && json.isNotEmpty) {
      for (final row in json) {
        final value = EdFiContactAddress.fromJson(row);
        if (value != null) {
          result.add(value);
        }
      }
  ...
  static Map<String, EdFiContactAddress> mapFromJson(dynamic json) {
    final map = <String, EdFiContactAddress>{};
    if (json is Map && json.isNotEmpty) {
      json = json.cast<String, dynamic>(); // ignore: parameter_assignments
      for (final entry in json.entries) {
        final value = EdFiContactAddress.fromJson(entry.value);
        if (value != null) {
          map[entry.key] = value;
        }
      }
  ...
  // maps a json object with a list of EdFiContactAddress-objects as value to a dart map
  static Map<String, List<EdFiContactAddress>> mapListFromJson(dynamic json, {bool growable = false,}) {
    final map = <String, List<EdFiContactAddress>>{};
    if (json is Map && json.isNotEmpty) {
      // ignore: parameter_assignments
      json = json.cast<String, dynamic>();
      for (final entry in json.entries) {
        map[entry.key] = EdFiContactAddress.listFromJson(entry.value, growable: growable,);
      }
    }
    return map;
  }

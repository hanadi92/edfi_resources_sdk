//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:edfi_resources_sdk/api.dart';
import 'package:test/test.dart';

// tests for EdFiStudentAssessmentRegistrationBatteryPartAssociation
void main() {
  // final instance = EdFiStudentAssessmentRegistrationBatteryPartAssociation();

  group('test EdFiStudentAssessmentRegistrationBatteryPartAssociation', () {
    // 
    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // EdFiAssessmentBatteryPartReference assessmentBatteryPartReference
    test('to test the property `assessmentBatteryPartReference`', () async {
      // TODO
    });

    // EdFiStudentAssessmentRegistrationReference studentAssessmentRegistrationReference
    test('to test the property `studentAssessmentRegistrationReference`', () async {
      // TODO
    });

    // An unordered collection of studentAssessmentRegistrationBatteryPartAssociationAccommodations. The special variation(s) to be used for the specific part of the assessment battery on how is presented, how it is administered, or how the test taker is allowed to respond.
    // List<EdFiStudentAssessmentRegistrationBatteryPartAssociationAccommodation> accommodations (default value: const [])
    test('to test the property `accommodations`', () async {
      // TODO
    });

    // A unique system-generated value that identifies the version of the resource.
    // String etag
    test('to test the property `etag`', () async {
      // TODO
    });

    // The date and time the resource was last modified.
    // DateTime lastModifiedDate
    test('to test the property `lastModifiedDate`', () async {
      // TODO
    });


  });

}

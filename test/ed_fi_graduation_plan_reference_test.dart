//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:edfi_resources_sdk/api.dart';
import 'package:test/test.dart';

// tests for EdFiGraduationPlanReference
void main() {
  // final instance = EdFiGraduationPlanReference();

  group('test EdFiGraduationPlanReference', () {
    // The identifier assigned to an education organization.
    // int educationOrganizationId
    test('to test the property `educationOrganizationId`', () async {
      // TODO
    });

    // The type of academic plan the student is following for graduation.
    // String graduationPlanTypeDescriptor
    test('to test the property `graduationPlanTypeDescriptor`', () async {
      // TODO
    });

    // The school year the student is expected to graduate.
    // int graduationSchoolYear
    test('to test the property `graduationSchoolYear`', () async {
      // TODO
    });

    // Link link
    test('to test the property `link`', () async {
      // TODO
    });


  });

}

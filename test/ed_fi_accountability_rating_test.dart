//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.18

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:edfi_resources_sdk/api.dart';
import 'package:test/test.dart';

// tests for EdFiAccountabilityRating
void main() {
  // final instance = EdFiAccountabilityRating();

  group('test EdFiAccountabilityRating', () {
    // 
    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // The title of the rating.
    // String ratingTitle
    test('to test the property `ratingTitle`', () async {
      // TODO
    });

    // EdFiEducationOrganizationReference educationOrganizationReference
    test('to test the property `educationOrganizationReference`', () async {
      // TODO
    });

    // EdFiSchoolYearTypeReference schoolYearTypeReference
    test('to test the property `schoolYearTypeReference`', () async {
      // TODO
    });

    // An accountability rating level, designation, or assessment.
    // String rating
    test('to test the property `rating`', () async {
      // TODO
    });

    // The date the rating was awarded.
    // DateTime ratingDate
    test('to test the property `ratingDate`', () async {
      // TODO
    });

    // The organization that assessed the rating.
    // String ratingOrganization
    test('to test the property `ratingOrganization`', () async {
      // TODO
    });

    // The program associated with the accountability rating (e.g., NCLB, AEIS).
    // String ratingProgram
    test('to test the property `ratingProgram`', () async {
      // TODO
    });

    // A unique system-generated value that identifies the version of the resource.
    // String etag
    test('to test the property `etag`', () async {
      // TODO
    });

    // The date and time the resource was last modified.
    // DateTime lastModifiedDate
    test('to test the property `lastModifiedDate`', () async {
      // TODO
    });


  });

}
